...
class ReplaceFstImpl
 ...
 public:
  ...
  bool ComputeArc(const StateTuple &tuple, const Arc &arc, Arc *arcp,
    ...
    const auto min_nonterminal = *nonterminal_set_.begin();
    const auto max_nonterminal = *nonterminal_set_.rbegin();
    if (arc.olabel == 0 || arc.olabel < min_nonterminal ||
        arc.olabel > max_nonterminal) {  // Expands local FST.
